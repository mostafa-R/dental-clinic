...
export default function CommandPalette() {
  ...
  const results = useMemo(() => {
    ...
    const tenantItems = tenants
      ...
      .filter((i) => !seen.has(i.id));
    tenantItems.forEach((i) => seen.add(i.id));
    const branchItems = branches
      .map((b) => {
        ...
        return {
          ...
          type: "branch",
          label: b.name,
          sublabel: b.tenant?.name || "",
          href: tenantId ? `/tenants/${tenantId}` : "/branches",
          group: "searchBranchesGroup",
        };
      })
      .filter((i) => !seen.has(i.id));
    branchItems.forEach((i) => seen.add(i.id));
    const userItems = users
      .map((u) => {
        ...
        return {
          ...
          type: "user",
          label: u.name,
          sublabel: u.email || u.tenant?.name || "",
          href: tenantId ? `/tenants/${tenantId}` : "/tenants",
          group: "searchUsersGroup",
        };
      })
      .filter((i) => !seen.has(i.id));
    return [...actionItems, ...recentItems, ...pageItems, ...tenantItems, ...branchItems, ...userItems];
  }, [filteredActions, recentItems, rankedPages, tenants, branches, users, language]);
  ...
  const canSeeBranches = canUserAccess(user, "branches");
  const canSeeUsers = canUserAccess(user, "tenants");
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(async () => {
      try {
        const calls = [
          api.get("/tenants", { params: { search: trimmed, page: 1, limit: 8 } }),
        ];
        if (canSeeBranches) {
          calls.push(api.get("/branches", { params: { search: trimmed, page: 1, limit: 6 } }));
        }
        if (canSeeUsers) {
          calls.push(api.get("/users", { params: { search: trimmed, limit: 6 } }));
        }
        const settled = await Promise.allSettled(calls);
        if (queryRef.current.trim() !== trimmed) return;
        let i = 0;
        const tenantsRes = settled[i++];
        setTenants(
          tenantsRes.status === "fulfilled" ? tenantsRes.value.data.tenants || [] : [],
        );
        if (canSeeBranches) {
          const branchesRes = settled[i++];
          setBranches(
            branchesRes.status === "fulfilled" ? branchesRes.value.data.branches || [] : [],
          );
        } else {
          setBranches([]);
        }
        if (canSeeUsers) {
          const usersRes = settled[i++];
          setUsers(
  ...
  const itemIcon = (item) => {
    ...
    if (item.type === "branch") return <Squares2X2Icon className="w-4 h-4 text-slate-400 shrink-0" />;
    if (item.type === "user") return <UserCircleIcon className="w-4 h-4 text-slate-400 shrink-0" />;
    return null;
  };
