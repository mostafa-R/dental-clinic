...
export const logout = createAsyncThunk("auth/logout", async () => {
  try {
    ...
    // Always purge client-side sensitive traces, even if the server call
    // failed — HttpOnly cookies are expired server-side, readable cookies
    // (_csrf) + sessionStorage are cleared here.
    const { clearClientSessionTraces, disconnectSiteSocket } = await import(
      "../../lib/realtime"
    );
    disconnectSiteSocket();
