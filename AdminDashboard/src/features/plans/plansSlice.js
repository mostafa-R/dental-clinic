...
export const fetchPlanModules = createAsyncThunk(
  "plans/fetchPlanModules",
  async (_, { rejectWithValue }) => {
    try {
      const response = await api.get("/plans/modules");
      return response.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to fetch plan modules",
      );
    }
  },
);
...
const plansSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchPlanModules.fulfilled, (state, action) => {
        state.modulesLoading = false;
        state.availableModules = Array.isArray(action.payload?.modules)
          ? action.payload.modules
          : [];
      })
      .addCase(fetchPlanModules.rejected, (state, action) => {
        state.modulesLoading = false;
