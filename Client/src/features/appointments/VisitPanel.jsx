        ...
        <div className="flex items-center gap-3">
          ...
          {canOpenEmr && patient?._id && (
            <button
              ...
              className="rounded-md bg-brand/5 px-2 py-1 text-xs font-medium text-brand-dark transition hover:bg-brand/10 dark:bg-brand/20 dark:text-brand-light dark:hover:bg-brand/25"
            >
              {t('emr.open')}
            </button>
          )}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <input type="number" min="0" step="0.01" value={invDiscount} onChange={(e) => setInvDiscount(e.target.value)} className="w-16 rounded border border-slate-200 bg-white px-2 py-1 text-xs text-right outline-none focus:border-brand dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
                  </label>
                  <label className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                    {t('billing.form.tax')}
                    <input type="number" min="0" step="0.01" value={invTax} onChange={(e) => setInvTax(e.target.value)} className="w-16 rounded border border-slate-200 bg-white px-2 py-1 text-xs text-right outline-none focus:border-brand dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
                  </label>
              ...
              <div className="flex flex-wrap gap-2">
                {options.map((opt) => (
                  <button
                    ...
                    className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-brand/5 hover:text-brand-dark disabled:opacity-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700"
                  >
    ...
    <Modal open onClose={onClose} title={t('emr.rx.new')} size="sm" zIndex={60}>
      <div className="space-y-3">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="button" onClick={submit} disabled={saving || !medication.trim()} className="rounded-lg bg-brand px-3 py-1.5 text-xs font-medium text-white transition hover:bg-brand-dark disabled:opacity-50">
