  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canManage && (
          <button type="button" onClick={openCreate} className="rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white transition hover:bg-brand-dark dark:bg-brand dark:hover:bg-brand-dark">
            {t('emr.note.new')}
          </button>
        )}
      </div>
        ...
        <div className="relative space-y-4 ps-4">
          ...
          {notes.map((note) => (
            <div key={note._id} className="relative">
              <span className="absolute start-[-5px] top-3 h-2.5 w-2.5 rounded-full bg-brand ring-4 ring-brand/10 dark:ring-brand/20" aria-hidden="true" />
              <Card>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs text-slate-400 dark:text-slate-500">{note.noteNo}</span>
                      <span className="rounded-full bg-brand/5 px-2 py-0.5 text-xs font-medium text-brand-dark dark:bg-brand/20 dark:text-brand-light">
