...
export default function TreatmentPlansTab({ patientId }) {
  ...
  const inputCls =
    'rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm text-slate-900 outline-none focus:border-brand focus:ring-2 focus:ring-brand/20 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:focus:ring-brand/20';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canManage && (
          <button type="button" onClick={() => setFormOpen(true)} className="rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white transition hover:bg-brand-dark dark:bg-brand dark:hover:bg-brand-dark">
            {t('emr.plan.new')}
          </button>
        )}
      </div>
              ...
              <Card key={plan._id} padded={false}>
                <div className="flex flex-wrap items-center gap-3 p-4">
                  ...
                  {canManage && plan.status !== 'archived' && plan.items.some((i) => i.status === 'completed' || i.status === 'in_progress') && (
                    ...
                    }} className="rounded-md px-2 py-1 text-xs font-medium text-brand transition hover:bg-brand/5 dark:text-brand-light dark:hover:bg-brand/20">
                    ...
                    <div className="space-y-1 p-4">
                      ...
                      {canManage && plan.status !== 'archived' && (
                        <div className="mt-2 flex flex-wrap items-center gap-2 rounded-lg border border-dashed border-slate-200 p-2 dark:border-slate-700">
                          ...
                          <button type="button" onClick={() => onAddItem(plan._id)} className="rounded-lg bg-brand px-3 py-1.5 text-xs font-medium text-white transition hover:bg-brand-dark dark:bg-brand">
                            {t('emr.plan.addItem')}
                          </button>
                        </div>
                      )}
