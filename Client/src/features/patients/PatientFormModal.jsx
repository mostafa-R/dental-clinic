  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between">
        ...
        <button type="button" onClick={add} className="text-xs font-medium text-brand hover:text-brand-dark dark:text-brand-light">
          {addLabel}
        </button>
      </div>
        ...
        {items.map((item, i) => (
          <div key={i} className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-slate-300 px-3 py-1.5 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/20 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
            />
            <button
              type="button"
              onClick={() => remove(i)}
