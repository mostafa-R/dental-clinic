      ...
      <Modal open={open} title={t('users.form.credentialsTitle') || 'Staff Created'} onClose={onClose} size="md">
        <div className="space-y-4">
          ...
          <div className="flex justify-end">
            <button type="button" onClick={() => { setCredentials(null); onClose(); }} className="rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white transition hover:bg-brand-dark dark:bg-brand dark:hover:bg-brand-dark">{t('common.done') || 'Done'}</button>
          </div>
        </div>
      </Modal>
    );
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button type="button" onClick={submit} disabled={formStatus === 'loading'} className="rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white transition hover:bg-brand-dark disabled:opacity-50 dark:bg-brand dark:hover:bg-brand-dark">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            <input type="checkbox" id="userIsActive" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} className="h-4 w-4 rounded border-slate-300 text-brand focus:ring-brand/40" />
            <label htmlFor="userIsActive" className="text-sm text-slate-600 dark:text-slate-300">{t('users.form.active')}</label>
          </div>
          <div className="flex items-center gap-3">
            <input type="checkbox" id="userIsDoctor" checked={isDoctor} onChange={(e) => setIsDoctor(e.target.checked)} className="h-4 w-4 rounded border-slate-300 text-brand focus:ring-brand/40" />
