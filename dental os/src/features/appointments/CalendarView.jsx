...
function resourceFor(a) { return a.chair || a.doctor?.name || 'Unassigned'; }
...
function AppointmentBlock({ appointment, conflict, onEdit, t }) {
  ...
  return (
    <button type="button" onClick={() => onEdit(appointment)}
      ...
      style={{ top, height }} aria-label={`${appointment.patient?.fullName || t('appointments.patientFallback')}, ${timeLabel(appointment.start)} to ${timeLabel(appointment.end)}${conflict ? ', schedule conflict' : ''}`}>
      ...
      {conflict && <span className="block truncate text-[10px] font-semibold leading-3">Conflict</span>}
    </button>
  );
}
...
function DaySchedule({ day, appointments, onEdit, onNew, t }) {
  ...
  const resources = useMemo(() => [...new Set(items.map(resourceFor))].sort(), [items]);
  ...
  const columns = resources.length ? resources : ['Unassigned'];
  ...
  return <section className="min-w-[720px] border-b border-slate-200 last:border-0 dark:border-slate-700">
    ...
    <div className="grid" style={{ gridTemplateColumns: `64px repeat(${columns.length}, minmax(170px, 1fr))` }}>
      ...
      {columns.map((resource) => <div key={resource} className="relative border-s border-slate-200 dark:border-slate-700" style={{ height, backgroundImage: `repeating-linear-gradient(to bottom, transparent 0, transparent ${PIXELS_PER_HOUR - 1}px, rgba(148,163,184,.22) ${PIXELS_PER_HOUR}px)` }}>{items.filter((a) => resourceFor(a) === resource).map((a) => <AppointmentBlock key={a._id} appointment={a} conflict={conflicts.has(a._id)} onEdit={onEdit} t={t} />)}</div>)}
    </div>
  </section>;
}
