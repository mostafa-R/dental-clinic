...
export default function MessageList() {
  ...
  return (
    <div role="log" aria-live="polite" aria-label={t('chat.title')} className="flex-1 overflow-y-auto p-4 space-y-2">
      {messages.map((msg) => {
        ...
        return (
          <div key={msg._id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[75%] rounded-lg px-3 py-2 break-words ${bubbleClass}`}>
              ...
              <p className={`mt-1 text-xs ${isOwn ? 'text-white/70' : 'text-slate-400'}`}>
