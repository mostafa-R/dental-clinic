...
export default function ClinicalTimelineTab({ patientId, patient }) {
  ...
  return (
    <div className="space-y-6">
                    ...
                    {note.attachments.map((att, i) => (
                      ...
                      >
                        {att.type === 'xray' || att.type === 'photo' ? (
                          <img src={att.url} alt={att.caption || ''} className="h-24 w-24 object-cover transition group-hover:opacity-80" loading="lazy" />
                        ) : (
                          <div className="flex h-24 w-24 flex-col items-center justify-center gap-1 bg-slate-50 text-slate-400 dark:bg-slate-800 dark:text-slate-500">
      ...
      <ClinicalNoteFormModal open={formOpen} patientId={patientId} patient={patient} onClose={() => setFormOpen(false)} />
    </div>
  );
}
